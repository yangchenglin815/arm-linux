...
int TermSavew(int nIndex,string testvalue,string testtime,string lng,string lat,string templ,string humidity,string termbattery,string devbattery,string unit,int gpsflag,int rssi,int nbr,string gpscnt);
...
{   //cout<<"procdata"<<endl;
	...
	{   
		...
		else if(pStr[1]=="CMD_DATA")
		{
			...
			TermSavew(nIndex,pStr[5],pStr[6],pStr[7],pStr[8],pStr[9],pStr[10],pStr[11],pStr[12],pStr[13],gpsflag,rssi,nbr,pStr[17]);
...
int UpdateTermNbr(int nIndex)
{
	...
	CHECKINDEX0()
		char sql[1024];
	char TmpStr[1024];
	sprintf(sql,"update ROMS200_CZTERM set nbr = 0 where id = '%s' ",skt_client_info[nIndex].m_nTermID);
	LogMesg(-1,sql);
	try
	{
...
int UpdateNbr(int nIndex,int nbr)
{
    ...
    char sql[1024];
	char TmpStr[1024];
	sprintf(sql,"update ROMS200_CZTASK set nbr = %d where id = '%s' ",nbr,skt_client_info[nIndex].m_nTaskID);
	LogMesg(-1,sql);
	try
	{
		otl_stream otl_update(1, sql,*g_dbCfg[nDBIndex].m_db);
	...
	sprintf(sql,"update ROMS200_CZTERM set nbr = %d where id = '%s' ",nbr,skt_client_info[nIndex].m_nTermID);
	LogMesg(-1,sql);
	try
	{
		otl_stream otl_update(1, sql,*g_dbCfg[nDBIndex].m_db);
...
int TermUpdate(int nIndex, string workway, string vnumber)
{
    ...
    char sql[1024];
	char TmpStr[1024];
	sprintf(sql,"update ROMS200_CZTERM  set vnumber = '%s' and workway = '%s' where id = '%s'  ",vnumber.c_str(),workway.c_str(),skt_client_info[nIndex].m_nTermID);
		LogMesg(-1,sql);
		try
		{
			otl_stream otl_update(1, sql,*g_dbCfg[nDBIndex].m_db);
...
int Updategps(int nIndex,string lng,string lat,int gpsflag)
{
	...
	sprintf(sql,"update ROMS200_CZTASKVALUE  set lng = '%s',lat = '%s',gpsflag = 1 where gpsflag = 0",newLng.c_str(),newLat.c_str());
	LogMesg(-1,sql);
	try
	{
		otl_stream otl_update(1, sql,*g_dbCfg[nDBIndex].m_db);
...
int UpdateOffline( )
{
	...
	CHECKINDEX0()
		...
		char sql[1024];	
	sprintf(sql,"update ROMS200_CZTERM  set online = 0  ");
	LogMesg(-1,sql);
	try
	{
		otl_stream otl_update(1, sql,*g_dbCfg[nDBIndex].m_db);
...
#else
	cfg.OpenIniFile("/usr/roms/config/CZ_fh40g.ini");
#endif
	char TmpStr[1024];
...
//	char logdata[1024];
	...
	UpdateGPS(lng,lat,newLng,newLat);
/*	sprintf(sql,"select companyid from CZ_TERM where id = '%s' ", skt_client_info[nIndex].m_nTermID);
	LogMesg(nIndex,sql);
	try
	{
	...
	//sprintf(sql,"INSERT INTO CZ_TASK(id,createtime,humidity,lat,lng,measuretime,taskname,temperature,companyid,termid) values(uuid(),now(),'%s','%s','%s','%s','%s','%s','%s','%s')",humidity.c_str(),newLat.c_str(),newLng.c_str(),testtime.c_str(),taskname.c_str(),templ.c_str(),companyid,skt_client_info[nIndex].m_nTermID);
	  sprintf(sql,"INSERT INTO ROMS200_CZTASK(id,createtime,taskname,ctermid,nbr) values(uuid(),now(),'%s','%s',%d)",taskname.c_str(),skt_client_info[nIndex].m_nTermID,nbr);
	...
	sprintf(sql,"select id from ROMS200_CZTASK where taskname= '%s' ", taskname.c_str());
	LogMesg(nIndex,sql);
	try
	{
		otl_stream otl_selectcc(1, sql,*g_dbCfg[nDBIndex].m_db);
...
int TermCheck(int nIndex,string sNumber,char * tID)
{
    ...
    char sql[1024];
	sprintf(sql,"select id  from ROMS200_CZTERM where termnumber = '%s' ",sNumber.c_str());
	LogMesg(nIndex,sql);
	try
	{
		otl_stream otl_selectcc(1, sql,*g_dbCfg[nDBIndex].m_db);
...
int update_term(int nIndex,string lat,string lng,string termbattery,string testvalue)
{
	...
	UpdateGPS(lng,lat,newLng,newLat);
	sprintf(sql,"update ROMS200_CZTERM SET lat='%s', lng='%s', powervalue='%s', testvalue='%s' where id = '%s'",newLat.c_str(),newLng.c_str(),termbattery.c_str(),testvalue.c_str(),skt_client_info[nIndex].m_nTermID);
...
{   //cout<<"TERMCHECKNEWTASK"<<endl;
	...
	try
	{
		...
		{ 
			otl_selectcc>>taskid;	
			//判断时间是否在同一天
			sprintf(sql,"SELECT  TO_DAYS(NOW())- TO_DAYS(createtime) FROM ROMS200_CZTASK where taskname = '%s' order by createtime desc limit 0,1 ",taskname.c_str());
			LogMesg(nIndex,sql);
			try
			{
				otl_stream otl_selectcc(1, sql,*g_dbCfg[nDBIndex].m_db);
...
{   //cout<<"TermOnline"<<endl;
	...
	{
		sprintf(sql,"update ROMS200_CZTERM  set online = %d where id = '%s'  ",on,skt_client_info[nIndex].m_nTermID);
		LogMesg(-1,sql);
		try
		{
			otl_stream otl_update(1, sql,*g_dbCfg[nDBIndex].m_db);
...
{  // cout<<"TERMSAVEW"<<endl;
	...
	//插入历史表
	sprintf(sql,"insert into ROMS200_CZTASKVALUE(id,createtime,devpower,humidity,lat,lng,temperature,termpower,testvalue,taskid,pieces,gpsflag,ctermid,nbr,currenttime)values (uuid(),'%s','%s','%s','%s','%s','%s','%s',%s,'%s','%s',%d,'%s',%d,now())",testtime.c_str(),devbattery.c_str(),humidity.c_str(),newLat.c_str(),newLng.c_str(),templ.c_str(),termbattery.c_str(),testvalue.c_str(),skt_client_info[nIndex].m_nTaskID,unit.c_str(),gpsflag,skt_client_info[nIndex].m_nTermID,nbr);
